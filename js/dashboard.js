(()=>{
  ...
  function renderMonths(m){
    const el=document.getElementById('monthlyChart'),vals=Object.values(m).flatMap(x=>[x.entrada,x.salida]),max=Math.max(1,...vals);
    const names=['Ene','Feb','Mar','Abr','May','Jun','Jul','Ago','Sep','Oct','Nov','Dic'];
    el.innerHTML=names.map((n,i)=>{const x=m[i+1]||{entrada:0,salida:0};return `<div class="month-group"><div class="bars"><i class="bar entry" style="height:${Math.max(3,x.entrada/max*100)}%" title="${App.money(x.entrada)}"></i><i class="bar output" style="height:${Math.max(3,x.salida/max*100)}%" title="${App.money(x.salida)}"></i></div><small>${n}</small></div>`}).join('')
  }
