...
if (auth_uses_demo()) {
    ...
    $_SESSION['user'] = [
        ...
        'email' => '',
        'position' => 'Revisión de Presupuesto',
        'department_id' => null,
        'legacy_roles' => [],
        'budget_permissions' => [['role'=>'ADMIN','department_id'=>null]],
        'auth_mode' => 'demo',
    ];
    demo_reset_store();
    json_response(['ok'=>true,'data'=>['user'=>$_SESSION['user'],'csrf'=>$_SESSION['csrf'],'mode'=>'demo']]);
}
...
$con = conectar();
if (!$con) json_response(['ok' => false, 'error' => 'No fue posible conectar a la base de datos desde este App Service'], 503);
...
$sql = "SELECT
        ...
        LIMIT 1";
$st = $con->prepare($sql);
if (!$st) { $con->close(); json_response(['ok'=>false,'error'=>'No se pudo preparar la consulta de autenticación'],500); }
...
if (!password_verify($password, (string)$acc['password_hash'])) {
    ...
    $cid = (int)$acc['cuenta_id'];
    $st->bind_param('ii', $fallidos, $cid); $st->execute(); $st->close();
    $con->close();
    json_response(['ok' => false, 'error' => 'Usuario o contraseña inválidos'], 401);
}
...
$con->close();
if (!$permissions) json_response(['ok'=>false,'error'=>'Tu cuenta no tiene permiso para el módulo de Presupuesto'],403);
...
json_response(['ok'=>true,'data'=>['user'=>$_SESSION['user'],'csrf'=>$_SESSION['csrf'],'mode'=>'db']]);
