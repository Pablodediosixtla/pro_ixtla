<?php
require_once dirname(__DIR__,2).'/lib/bootstrap.php';require_method('POST');require_csrf();$user=require_login();
$isMultipart=str_contains(strtolower((string)($_SERVER['CONTENT_TYPE']??'')),'multipart/form-data');$in=$isMultipart?$_POST:json_input();$type=strtoupper(trim((string)($in['tipo']??'SALIDA')));if(!in_array($type,['ENTRADA','SALIDA'],true))json_response(['ok'=>false,'error'=>'Tipo de movimiento inválido'],400);if($type==='SALIDA'&&!user_has_permission($user,'MOVIMIENTO_SALIDA_CREAR'))json_response(['ok'=>false,'error'=>'Solo Tesorería, Presidencia o Administración pueden registrar salidas reales'],403);if($type==='ENTRADA'&&(!user_has_permission($user,'MOVIMIENTO_ENTRADA_CREAR')||!user_has_any_role($user,['ADMIN','PRESIDENTE','TESORERIA'])))json_response(['ok'=>false,'error'=>'Solo Administración, Presidencia o Tesorería pueden registrar entradas'],403);
$db=conectar();if(!$db)json_response(['ok'=>false,'error'=>'Sin conexión a base de datos'],503);$requestId=(int)($in['solicitud_id']??0);$request=null;
